...
#' @import SNPRelate
#' @import SeqArray
.rowRanges_gdsdata <- function(file, fileFormat, rowDataColumns, infoColumns, rowDataOnDisk){
...
makeSummarizedExperimentFromGDS <- function(file, name=NA, rowDataColumns=character(), colDataColumns=character(), infoColumns=character(), fmt=character(), rowDataOnDisk=TRUE, colDataOnDisk=TRUE, fmtDataOnDisk=TRUE){
    ...
    rowRange <- .rowRanges_gdsdata(file, fileFormat=ff, rowDataColumns = rowDataColumns, infoColumns = infoColumns, rowDataOnDisk = rowDataOnDisk)
