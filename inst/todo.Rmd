...
- modify `GDSArraySeed` and `GDSArray` to accommodate ordinary gds format data, not just `SNP_ARRAY` or `SEQ_ARRAY`. 
+ `.get_gdsdata_fileFormat` no change. But remember to add the `put.attr.gdsn(FileFormat="SE_ARRAY")` in `.write_gds_assays`.  -- done.   
+ `.get_gdsdata_arrayNodes` no change.
+ `.read_gdsdata_sampleInCol` adding `fileFormat =="SE_ARRAY"`. -- done.
+ `.get_gdsdata_dim` no change. 
+ `.get_gdsdata_dimnames` adding `fileFormat =="SE_ARRAY"` to accommodate the following change: add the `sample.id` and `row.id` in `.write_gds_assays` when creating the gds file from SE assay. 
```
rownames(colData(se))
names(rowData(se))
should be easier way to extract the dimnames of SE. 
```
- modify `makeSummarizedExperimentFromGdsfmt`?? 
...
class(asy)
## hdf5 format
SummarizedExperiment::saveHDF5SummarizedExperiment(se)
dir("my_h5_se")
load("my_h5_se") ### errors
load("my_h5_se/se.rds")  ### errors
saveGdsfmtSummarizedExperiment(se)
## use saveGdsfmtSummarizedExperiment() to rewrite into DelayedArray, and save .rds.
