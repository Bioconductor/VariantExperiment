...
- modify "show" method accordingly for nrows() and rownames().   -- done!
- [[]] extraction, modify to be consistent with dim(ddf). 
  -- lower level function: "getListElement("DelayedDataFrame")" defined. 
- lazyIndex@listData. do not distinguish (un)delayed columns, use only vector for index. ?? 
- "check\_lazyIndex\_in_use" and reorder.   -- when??
- c(LazyList), add checking validity. 
- rbind, cbind. c(DDF, DDF)
