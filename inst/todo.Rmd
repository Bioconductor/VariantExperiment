...
## 7. todo: 
- add `INFO` from `SeqArray` data for `rowDataColumns`. 
- Remove the `allow.duplicate` from arguments. 
- add more manual context for `GDSArray-class.R`. Why doesn't load packages???

## 8. querable gds data. return the allowable values in the gds files for rowDataColumns and colDataColumns. 
coversion from SE to SeqVarGDSClass. to accommodate with SeqArray functions. 

11/27/2017, meeting with Martin
```{r}
x <- 1:10
.Internal(inspect(x))
