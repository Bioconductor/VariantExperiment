5/29/2018
- finalize "VCF2VE()" function. 
  - check seqVCF2GDS arguments: header, start, count, optimize, raise.error, digest, paralle.
  - test VCF2VE() arguments. -- done! 
	"verbose=FALSE" by default. 
  - add argument for sample info.    -- done!
	-- no blank line.
	-- with rownames() for sample.id.
	-- warning if any sample ID is not matched?? 
  - modify ".info_seqgds" for arbitray info columns (now from gds to VE, no problem, how about from vcf into gds?).  -- no problem!
  
- if gdsnode in "data.frame" or "list", the individual elements could show in a hierarchical structure. but here it would be easier to save sample annotation as gds folder node, and each info element as gds node.   -- figured!
- colData() reading "sample.annotation", "integer" converted into "character"?  -- checked. as.array() wil return "type() = integer". 
- "showAvailable" for rowData? (fixed entries now..., try a different vcf file)  -- done!
  - check fixed header columns (tab-delimited) for vcf files. 
	8 fixed, mandatory cols: #CHROM, POS, ID, REF, ALT, QUAL, FILTER, INFO
	if genotype data present, follow by FORMAT, then arbtrary number of sample IDs. 

- test stats functions with real VCF data.   -- doing...
  -- save any results into VE? seqAlleleFrequency, seqAlleleCount, seqMissing(variant/sample)...NO!
...
- add testthat functions for VCF2VE and other stats functions.
  -- "test\_VariantExperiment-stat.R". -- done!
  -- prepare a example sample info file and save in "inst/extdata/sample.info", for test. -- done!
  -- matrix values returned from stats functions, return as GDSArray? or ordinary array? -- ordinary array for now!
	  "refDosage", "altDosage", ...same dimension as genotype array. 
...
- FIXME: 
  -- altDosage(ve)  ## FIXME: sparse=TRUE does not work...    -- done! 
  -- missingGenotypeRate (FIXME: same as seqMissing remove it!!)  -- done!
  -- "meanBySample", append "/data" for the "var.name", i.e., names(assay(ve)). 
	  -- Add restrictions in the @param var.name !!! 
  -- return all matrix like output as the same dimension of ve. (refDosage, altDosage, isVariant...) -- done!
  -- where is the reference info saved in VE? gds attributes?
  -- import(SE): will this import classes and methods? 
  -- importFrom(SeqArray), to fix the conflicting functions, like colData(), rowRanges()...
  -- gdsfile(ve), return a vector? or just 1 path for all assay data? 
...
```{r}
BiocManager::available("mafdb")
library(AnnotationHub)
hub <- AnnotationHub()
query(hub, "mafdb")
query(hub, c("castelo", "phast"))
a <- query(hub, c("repeat", "homo"))
a["AH5122"]  ## RETURNS the description.
a[["AH5122"]] ## retrieves the data (download for the 1st time)

b <- query(hub, c("esp", "homo sapiens"))
mcols(b)
query(hub, "1000 genomes")
```
4/17/2018

- finalize the `DelayedDataFrame` constructor. When inputs are all DDF, call `concatenateObjects,DelayedDataFrame`. Otherwise, call `DataFrame` constructor, and coerce into `DelayedDataFrame`. -- done!
- update unit tests.   -- done!
- `GDSArray` failing building. -- fixed on 0.9.14.

4/3/2018

- "[" replace method. for multiple columns?   -- could only give values for 1 column... correct!
- add function to read in the vcf into VE directly! 
