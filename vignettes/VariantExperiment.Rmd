...
Use the function `saveGDSSummarizedExperiment` to save an SE object with ordinary in-memory array data into the SE object with GDSArray data. 
Question: Could we also save the `rowData` and `colData` as GDSArray? If that, the `rowData` and `colData` should also be saved as GDS node in the gds file, in order to use the function `GDSArray`. Create gds folder as `rowData` and `colData` to save the corresponding information. 

```{r, save}
### rewrite the DelayedArray data to be array/matrix.
assays(se)
f <- openfn.gds(file)
a1 <- aperm(read.gdsn(index.gdsn(f, "genotype/data")), perm=c(3,2,1))
a2 <- aperm(read.gdsn(index.gdsn(f, "phase/data")), perm=c(2,1))
a3 <- aperm(read.gdsn(index.gdsn(f, "annotation/format/DP/data")), perm=c(2,1))
closefn.gds(f)
assays(se) <- setNames(list(a1, a2, a3), names(assays(se)))
dim(assay(se, 1))
class(assay(se,1))
...
Use the function `loadGDSSummarizedExperiment` to load the newly generated GDSArray SE object. 
```{r, load}
se2 <- loadGDSSummarizedExperiment(dir="my_gds_se")
assay(se2, 1)
all.equal(assays(se1), assays(se2))
```
