...
The coercion function of `GDS2VE` could coerce `GDS` files into
...
for features and samples. All available data entries for `GDS2VE`
arguments could be retrieved by the `showAvailable()` function with
the gds file name as input. 
...
```{r GDS2VEArgs}
ve3 <- GDS2VE(gds, rowDataColumns = c("ID", "ALT", "REF"),
...
```{r gdsfile}
gdsfile(ve)
## a <- tempfile(fileext=".gds")    ## FIXME: slot setter for "gdsfile() <- "
## file.create(a)
## gdsfile(ve) <- a
```
...
##   'saveGDSSummarizedExperiment()' to synchronize on-disk and
##   in-memory representations
```

### save `VariantExperiment` object

Use the function `saveGDSSummarizedExperiment` to synchronize the
on-disk and in-memory representation.

```{r saveVE}
a <- tempfile()
saveGDSSummarizedExperiment(ve4, dir=a, replace=TRUE)
```
...
```{r loadVE}
ve4 <- loadGDSSummarizedExperiment(dir=a)
gdsfile(ve4)
```
